...
<xsl:template name="m:osgr2svg">
	...
	<xsl:variable name="legendPictureHg"  select="20"/>    <!--vyska obrazku v legende, musi byt mensi nez legendLineHg-->
	<xsl:variable name="legendGap"  select="5"/>
	<xsl:variable name="legendFontSize"  select="12"/>
	<xsl:variable name="legendFontWd"  select="0.61"/>
	<xsl:variable name="legendLineHg"  select="24"/>  <!--vyska radku v legende-->
...
<!--rozdeleni podle GRAPHTYPE-->
<xsl:choose>
<xsl:when test="$gra/ph/@graphType = 'pie'">  <!-- ***************graphType = 'pie'-->
	<!--vypocty promenych pro pie-->
		<!--samotny pie graf-->
	...
	<xsl:variable name="pieYCenter" select="$titleHg + $legendT + $pieTSpace + $pieRadiusY +
		...
		<!--umisteni polozek legendy u pie-->
	...
	<xsl:variable name="legendSY" select=" 
			...
			if ($gra/ph/@legend = 'top' or $gra/ph/@legend = 'botom') then (
				for $a in $gra/ph/names/name return (
					$titleHg + (if ($gra/ph/@legend = 'botom') then $graphHg else $legendMargin) +0.5*$legendLineHg
				)
			) else ''
			"/>
		<!--cely vyrez pie-->
	...
	<xsl:if test="(not ($gra/ph/@legend = 'none'))">
		...
		<!--ramecek kolem celeho grafu pie-->
	<svg:rect x="0.5" y="0.5" width="{$width - 1}" height="{$height - 1}"  
			stroke="black" fill="none" stroke-width="1"/>  
	</svg:svg> 
</xsl:when>
<xsl:otherwise>  <!--******************************graphType = 'norm' a ostatni-->
	<!--vypocty promenych pro norm-->
		<!--osa X - osa kategorii-->
	...
	<xsl:variable name="originYShift" select="
			if ($gra/ph/@xAxisPos = 'botom') then 0 else
	...
	<xsl:variable name="yShift"  select="$yAxisTStart + $yAxisHg - $yKoef * $yAxisMin"/>	
		...
		<!--umisteni polozek legendy u norm-->
	<xsl:variable name="legendSX" select=" 
			if ($gra/ph/@legend = 'right' or $gra/ph/@legend = 'left') then (
				for $a in $gra/ph/names/name return
					(if ($gra/ph/@legend = 'right') then $graphWd else $legendMargin)
	...
	<!-- promene pro tisk os a mrizek norm-->
	<xsl:variable name="LB" select="$gra/ph/@xAxisPos = 'botom'"/>
	...
	<!--zacatek svg dokumentu norm-->
	<svg:svg viewBox="0 0 {$width} {$height}"> 
	<svg:desc><xsl:value-of select="$gra/ph/title"/></svg:desc>  
	
	<!--vysazi nadpis grafu norm-->
	<svg:g>
	<xsl:if test="count($gra/ph/title) &gt; 0">
		<svg:text x="{$width div 2}" y="{$titleMargin + $titleFontSize}" 
				text-anchor="middle"
	...
	<xsl:if test="($gra/ph/@xGrid='minor' or $gra/ph/@xGrid='both')">    <!--vedlejsi mrizka osy x-->
	...
	<xsl:if test="($gra/ph/@xGrid !='none' and $gra/ph/@xGrid !='minor' )">    <!--hlavni mrizka osy x-->
	...
	<xsl:if test="$gra/ph/@yGrid = 'minor' or $gra/ph/@yGrid = 'both' "> <!--vedlejsi mrizka osy y-->
	...
	<xsl:if test="not ($gra/ph/@colType = 'none')"> 
		...
		<xsl:for-each select="$gra/ph/values/value">
			...
			<xsl:call-template name="m:drawCol"> <!-- vykresli sloupec-->
	...
	<svg:g stroke="black"> 
	<xsl:if test="(not ($gra/ph/@xAxisDivision='none' or $gra/ph/@xAxisDivision='major'))">    <!--vedlejsi znacky osy x-->
	...
	<xsl:if test="($gra/ph/@xAxisDivision='major' or $gra/ph/@xAxisDivision='both' )">    <!--hlavni znacky osy x-->
	...
	<!--ramecek kolem celeho grafu norm-->
	<svg:rect x="0.5" y="0.5" width="{$width - 1}" height="{$height - 1}"  
			stroke="black" fill="none" stroke-width="1"/>  
	<!-- kontrolni rameceky>
...
<xsl:function name="m:LineType"> <!-- vrátí dasharray odpovídající typu čáry-->
	<xsl:param name="t"/>
	<xsl:value-of select="
		...
		if ($t='longDash-dash') then '14,3,6,3' else '0,16'"/>
</xsl:function>

<xsl:template name="m:drawCol"> <!-- vykresli sloupec-->
	<xsl:param name="type"/>
	...
	<xsl:choose>
		<xsl:when test="$effect = '3D'">  <!--3D-->
			<xsl:choose>
				...
				<xsl:when test="$type = 'cone' ">  <!--3D cone ...opravit--> 
				...
				<xsl:when test="$type = 'pyramid' ">  <!--3D pyramid ...aby se tW=0 kreslilo zvlast-->
				...
				<xsl:otherwise> <!--3D block a ostatni-->
		...
		<xsl:otherwise>  <!--2D a ostatni-->
			<xsl:choose>
				...
				<xsl:otherwise> <!--2D block a ostatni-->
					<svg:path d="M{-$colW},{-$hg*(1-$bW)} h{2*$colW} v{-$hg*($bW -$tW)} h{-2*$colW} z"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:otherwise>
...
<xsl:function name="m:Arc"> 	<!-- vrati cestu, ktera vykresli rovnoosou pulelipsu v + smeru-->
	<xsl:param name="dx"/>
	<xsl:param name="hg"/>
	<xsl:param name="sp"/>
