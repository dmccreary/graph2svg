...
<xsl:template name="m:msgr2svg">
	...
	<xsl:variable name="gra">
		...
		<xsl:attribute name="legend" select="
			...
			if (($graph/@legend) = 'botom') then 'botom' else 'right' "/>
	...
	<xsl:variable name="legendY"  select="$titleHg + 
			...
			"/--> <!--pouziva se uz jenom na kontrolni ramecek, nahrazeno legendSX a SY-->
	...
	<xsl:if test="some $a in ($gra/ph/values/@fillArea, $gra/ph/@fillArea) satisfies ($a = 'yes')">
		...
		<xsl:for-each select="$gra/ph/values[./@fillArea = 'yes' or 
			...
			<xsl:variable name="v_"  select="preceding-sibling::values[1]"/>  <!-- predchozi rada-->
			<xsl:variable name="lp_"  select="min(($catCount, count($v_/value)))"/> <!-- pouzitelna delka predchozi rady-->
...
<xsl:template match="gr:value" mode="m:processValues">
	...
	<xsl:value-of select="
		...
		if ($graph/@stacked='percentage') then (
				sum((../preceding-sibling::values/value[$pos], .)) div sum(../../values/value[$pos]) ) else
