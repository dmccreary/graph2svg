                ...
                <svg:svg viewBox="0 0 {$width} {$height}">
                    ...
                    <svg:g>
                        <xsl:if test="count($gra/ph/title) &gt; 0">
                            <svg:text x="{$width div 2}" y="{$titleMargin + $titleFontSize}" text-anchor="middle" font-family="Verdana" font-size="{$titleFontSize}" fill="{if ($gra/ph/title/@color) then $gra/ph/title/@color else 'black'}">
                                <xsl:value-of select="$gra/ph/title"/>
                            </svg:text>
                        </xsl:if>
                    </svg:g>
                    ...
                    <svg:g stroke-width="1" stroke="black" stroke-linejoin="round">
                        <xsl:for-each select="$gra/ph/values/value">
                            ...
                            <svg:path d="{concat('M', $pieXCenter, ',', $pieYCenter,      ' l', $pieRadiusX*math:sin($sA), ',',  -$pieRadiusY*math:cos($sA),      ' a', $pieRadiusX, ',', $pieRadiusY, ' 0 ', $laf, ',1 ',       $pieRadiusX*(math:sin($eA) - math:sin($sA)), ',',      -$pieRadiusY*(math:cos($eA)  - math:cos($sA)), ' z')}" fill="{$cc}"/>
                    ...
                    <xsl:if test="$gra/ph/@effect = '3D'">
                        ...
                        <svg:path d="{concat('M', $pieXCenter +$pieRadiusX, ',', $pieYCenter,       ' v', $pie3DHg,       ' a', $pieRadiusX, ',', $pieRadiusY, ' 0 1,1 ', -2*$pieRadiusX, ',0',      ' v', -$pie3DHg,       ' a', $pieRadiusX, ',', $pieRadiusY, ' 0 1,0 ', 2*$pieRadiusX, ',0', ' z')}" fill="url(#lg_pie)" stroke="none"/>
                    </xsl:if>
                    ...
                    <xsl:if test="(not ($gra/ph/@labelIn = 'none'))">
                        <svg:g text-anchor="middle" font-family="Verdana" font-size="{$labelInFontSize}" fill="black">
                            <xsl:for-each select="$gra/ph/values/value">
                                ...
                                <svg:text x="{$pieXCenter + $pieRadiusX*$labelInRadiusRatio*math:sin($mA)}" y="{$pieYCenter -$pieRadiusY*$labelInRadiusRatio*math:cos($mA) + 0.35* $labelInFontSize}">
                                    <xsl:choose>
                    ...
                    <xsl:if test="(not ($gra/ph/@labelOut = 'none'))">
                        <svg:g font-family="Verdana" font-size="{$labelOutFontSize}" fill="black">
                            <xsl:for-each select="$gra/ph/values/value">
                                ...
                                <xsl:variable name="middleS" select="sum(preceding-sibling::value) + 0.5*(.)"/>
                                <xsl:variable name="mA" select="2*$pi*$middleS div $pieValSum"/>
                                <svg:text x="{$pieXCenter + ($pieRadiusX+0.85*$labelOutFontSize)*math:sin($mA)}" y="{$pieYCenter -($pieRadiusY + 0.85*$labelOutFontSize +        ( if (math:cos($mA) &lt; 0)  then $pie3DHg else 0 )      )*math:cos($mA) + 0.35* $labelOutFontSize}" text-anchor="{if ($mA &lt;= $pi) then 'start' else 'end'}">
                                    <xsl:choose>
                    ...
                    <xsl:if test="(not ($gra/ph/@legend = 'none'))">
                        <svg:g text-anchor="start" font-family="Verdana" font-size="{$legendFontSize}" fill="black">
                            <xsl:for-each select="$gra/ph/names/name">
                                ...
                                <!--xsl:variable name="cn"  select="$sn mod count($colorSch)+1"/>
			<xsl:variable name="cc"  select="if (@color) then @color else $colorSch[$cn]"/-->
                                <svg:text x="{$legendSX[1+$sn] + $legendPictureWd + $legendGap}" y="{$legendSY[1+$sn] + 0.35* $legendFontSize}">
                                    <xsl:value-of select="."/>
                                </svg:text>
                            </xsl:for-each>
                        </svg:g>
                    ...
                    <svg:rect x="0.5" y="0.5" width="{$width - 1}" height="{$height - 1}" stroke="black" fill="none" stroke-width="1"/>
                </svg:svg>
            </xsl:when>
            <xsl:otherwise>
                ...
                <svg:svg viewBox="0 0 {$width} {$height}">
                    ...
                    <xsl:if test="($gra/ph/@xGrid='minor' or $gra/ph/@xGrid='both')">
                        ...
                        <svg:path d="{$gXMinor}" stroke="{$minorGridColor}" stroke-width="{$minorGridStroke-width}" fill="none"/>
                    </xsl:if>
                    <xsl:if test="($gra/ph/@xGrid !='none' and $gra/ph/@xGrid !='minor' )">
                        ...
                        <svg:path d="{$gXMajor1}" stroke="{$majorGridColor}" stroke-width="{$majorGridStroke-width}" fill="none"/>
                    </xsl:if>
                    <xsl:if test="$gra/ph/@yGrid = 'minor' or $gra/ph/@yGrid = 'both' ">
                        ...
                        <svg:path d="{$gYMinor}" stroke="{$minorGridColor}" stroke-width="{$minorGridStroke-width}" fill="none"/>
                    </xsl:if>
                    <xsl:if test="($gra/ph/@yGrid = 'major' or $gra/ph/@yGrid = 'minor')     and ($yAxisDiv &gt; 0) ">
                        ...
                        <svg:path d="{$gYMajor}" stroke="{$majorGridColor}" stroke-width="{$majorGridStroke-width}" fill="none"/>
                    </xsl:if>
                    ...
                    <xsl:if test="not ($gra/ph/@colType = 'none')">
                        ...
                        <xsl:if test="($gra/ph/@colType = 'cone') or ($gra/ph/@colType = 'cylinder') ">
                            ...
                            <svg:defs>
                                <xsl:for-each select="$gra/ph/values/value">
                                    ...
                                    <svg:linearGradient id="lg{$vn}" x1="{$gradLBorder*$colWd -$pomS}" y1="0" x2="{$gradRBorder*$colWd -$pomS}" y2="0" gradientUnits="userSpaceOnUse">
                        ...
                        <svg:g stroke-width="0.4" stroke="black" stroke-linejoin="round">
                            <xsl:for-each select="$gra/ph/values/value">
                                ...
                                <svg:g transform="translate({m:R($x)}, {$y})" fill="{      if ($gra/ph/@colType = 'cone' or $gra/ph/@colType = 'cylinder') then       concat('url(#lg', $vn, ')') else $cc}">
                                ...
                                <xsl:if test="not ($gra/ph/@legend = 'none')">
                                    <svg:g transform="translate({$legendSX[1+$vn] + 0.5*$legendPictureWd},         {$legendSY[1+$vn] +0.5*$legendPictureHg})" fill="{if ($gra/ph/@colType = 'cone' or $gra/ph/@colType = 'cylinder') then        concat('url(#lg', $vn, ')') else $cc}">
                                        <xsl:call-template name="m:drawCol">
                                            <!-- draw a column-->
                                            <xsl:with-param name="type" select="$gra/ph/@colType"/>
                                            <xsl:with-param name="effect" select="$gra/ph/@effect"/>
                    ...
                    <!-- X axis with labels -->
                    <svg:line x1="{$xAxisLStart}" y1="{$originY}" x2="{$xAxisLStart + $xAxisWd}" y2="{$originY}" stroke="black" stroke-width="{$axesStroke-width}"/>
                    <!-- X axis labels -->
                    ...
                    <xsl:if test="$xLabelRotation">
                        <svg:g font-family="Verdana" font-size="{$labelFontSize}" fill="black">
                            <xsl:for-each select="$gra/ph/names/name">
                                <xsl:variable name="nn" select="count(preceding-sibling::name)"/>
                                <svg:g transform="translate({m:R($xAxisLStart + ($nn)*$catWd +$catGap)},       {m:R($originY + $majorMarkLen + $labelFontSize)}) rotate({$labelAngle}) ">
                                    <svg:text>
                                        <xsl:value-of select="."/>
                                    </svg:text>
                                </svg:g>
                    ...
                    <xsl:if test="$gra/ph/@lineType != 'none'">
                        ...
                        <svg:path d="{$line}" stroke="black" stroke-width="1.5" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    <xsl:if test="(not ($gra/ph/@labelIn = 'none'))">
                        <svg:g text-anchor="middle" font-family="Verdana" font-size="{$labelInFontSize}" fill="black">
                            <xsl:for-each select="$gra/ph/values/value">
                                <xsl:variable name="vn" select="count(preceding-sibling::value)"/>
                                <svg:text x="{$pX + $vn*$catWd}" y="{m:R( 0.5*($originY +$yShift +$yKoef * (.)) + 0.35*$labelInFontSize )}">
                                    <xsl:choose>
                                        <xsl:when test="$gra/ph/@labelIn = 'value' ">
                                            <xsl:value-of select="."/>
                                        </xsl:when>
                    ...
                    <xsl:if test="(not ($gra/ph/@labelOut = 'none'))">
                        <svg:g text-anchor="middle" font-family="Verdana" font-size="{$labelOutFontSize}" fill="black">
                            <xsl:for-each select="$gra/ph/values/value">
                                <xsl:variable name="vn" select="count(preceding-sibling::value)"/>
                                <svg:text x="{$pX + $vn*$catWd  +0.5*$depthX}" y="{m:R($yShift + $yKoef * (.) ) +        (if (. &gt;= 0) then (-4 -$depthY) else ($labelOutFontSize +2))}">
                                    <xsl:choose>
                                        <xsl:when test="$gra/ph/@labelOut = 'value' ">
                                            <xsl:value-of select="."/>
                                        </xsl:when>
            ...
            <xsl:when test="$effect = '3D'">
                ...
                <xsl:choose>
                    <xsl:when test="$type = 'cylinder' ">
                        <!--3D cylinder-->
                        <svg:path d="M{-$colW +0.5*$dpX},{-$hg*(1-$bW) -0.5*$dpY} {m:Arc($colW,0.5*$dpX,0)}        v{-$hg*($bW -$tW)} {m:Arc(-$colW,0.5*$dpX,1)} z"/>
                        <svg:path d="M{-$colW +0.5*$dpX},{-$hg*(1-$tW) -0.5*$dpY} {m:Arc($colW,0.5*$dpX,0)}        {m:Arc(-$colW,0.5*$dpX,0)}" fill="{$color}"/>
                    </xsl:when>
                    <xsl:when test="$type = 'cone' ">
                        <!--3D cone, TODO: not exact-->
                        <svg:path d="M{-$colW*$bW +0.5*$dpX},{-$hg*(1-$bW) -0.5*$dpY}        {m:Arc($colW*$bW,0.5*$dpX*$bW,0)}        l{-$colW*($bW -$tW)},{-$hg*($bW -$tW)}        {m:Arc(-$colW*$tW,0.5*$dpX*$tW,1)} z"/>
                        <svg:path d="M{-$colW*$tW +0.5*$dpX},{-$hg*(1-$tW) -0.5*$dpY}        {m:Arc($colW*$tW,0.5*$dpX*$tW,0)}        {m:Arc(-$colW*$tW,0.5*$dpX*$tW,0)} " fill="{$color}"/>
                    </xsl:when>
                    <xsl:when test="$type = 'pyramid' ">
                        <!--3D pyramid, TODO: to draw tW=0 separately -->
                        <svg:path d="M{-$colW*$bW +0.5*$dpX*(1-$bW)},{-$hg*(1-$bW) -0.5*$dpY*(1-$bW)}       h{2*$colW*$bW}        l{-$colW*($bW -$tW) +0.5*$dpX*($bW -$tW)},{-$hg*($bW -$tW) -0.5*$dpY*($bW -$tW)}       h{-2*$colW*$tW} z"/>
                        <svg:path d="M{$colW*$bW +0.5*$dpX*(1-$bW)},{-$hg*(1-$bW) -0.5*$dpY*(1-$bW)}       l{$dpX*$bW},{-$dpY*$bW}       l{-$colW*($bW -$tW) -0.5*$dpX*($bW -$tW)},{-$hg*($bW -$tW)+0.5*$dpY*($bW -$tW)}       l{-$dpX*$tW},{$dpY*$tW} z"/>
                        <svg:path d="M{-$colW*$tW +0.5*$dpX*(1-$tW)},{-$hg*(1-$tW) -0.5*$dpY*(1-$tW)}       h{2*$colW*$tW}        l{$dpX*$tW},{-$dpY*$tW}       h{-2*$colW*$tW} z"/>
                    </xsl:when>
                    <xsl:when test="$type = 'line' ">
                        <!-- 3D line -->
                        <svg:path d="M{0},{-$hg*(1-$bW)} v{-$hg*($bW -$tW)}" stroke-width="2" stroke-linecap="butt" stroke="{$color}"/>
                    </xsl:when>
                    <xsl:otherwise>
